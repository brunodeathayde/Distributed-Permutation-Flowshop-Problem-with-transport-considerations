...
# Test for CP
import pandas as pd
from CP_IBM import CP_IBM, read_instance
    ...
    for n in n_small:
        for m in m_small:
            for i in range(1, 6):
                ...
                UB, LB, GAP, Time, msol, status = CP_IBM(p, r, d, nf, TL=TL, objective=FO)
                ...
                if with_tt:
                    resultados.to_csv(f'Resultados_CP_ibm_DFm_r_{FO}.txt')
                else:
                    resultados.to_csv(f'Resultados_CP_ibm_DFm_{FO}.txt')
