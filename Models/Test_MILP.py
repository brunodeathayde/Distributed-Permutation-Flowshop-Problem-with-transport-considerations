...
# Test for CP
import pandas as pd
from MILP import MILP, read_instance

TL = 600
method = 'cplex' # cplex or gurobi
FO = 'Cmax' # Cmax, TotalCT, Tardiness or JIT
with_tt = True # With or without transportation times
...
print('Small Instances Test - MILP Model')
    ...
    for n in n_small:
        for m in m_small:
            for i in range(1, 6):
                ...
                UB, LB, GAP, Time, msol, status = MILP(p, r, d, nf, TL=TL, method=method, objective=FO)
                ...
                if with_tt:
                    resultados.to_csv(f'Resultados_MILP_{method}_DFm_r_{FO}.txt')
                else:
                    resultados.to_csv(f'Resultados_MILP_{method}_DFm_{FO}.txt')
                
print('Large Instances Test - MILP Model')
